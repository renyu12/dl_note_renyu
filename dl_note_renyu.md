...
    注意是所有z = wx + b的z，然后都是做$\frac{e^{z_i}}{e^{z_1}+e^{z_2}+e^{z_3}...}$这样，设计成这样的形式，一是需要概率形式，z的取值范围是正无穷到负无穷，取指数可以保证值一定是整数，二是需要保证同一层所有输出的概率合为1，所以加和作为分母，三是做了指数的效果是进一步放大了最大值压制了其他值，可以清楚地根据输出层输出最大的项确认分类。仔细看看softmax的损失函数，会发现当只有1项输出分类时，softmax的形式和sigmoid是一样的  
...
    inception v1版本还有5\*5的卷积核，计算成本实在是太高了，v2版本就移除改成2层3\*3了，并且提出了Batch Normalization（BN）方法，通过归一化解决梯度消失/爆炸的问题，简单来说的思路就是每一层都根据一部分训练数据归一化输入值，使其分布在能让激活函数输出在比较合适的区间，缓解梯度消失/爆炸，从而使得增加层数更容易收敛。TODO: BN层的具体实现研究下。2017年还更新了一版Xception。  
#### Resnet 2015第一名  
...
换一种“门控”的角度来理解，引入了长期记忆链路，可以理解为增加了三个门：输入门（控制 当前时刻输入+上一时刻隐藏状态 进入长期记忆）、遗忘门（根据 当前时刻输入+上一时刻隐藏状态 判断长期记忆遗忘哪些）、输出门（控制 长期记忆 输出到当前时刻隐藏状态）。即通过门控机制来控制需要额外输入、遗忘、输出什么。  
