...
class VisionMamba(nn.Module):
    def __init__(
        ...
        if self.model_type == "student":
            self.distill_head = nn.Sequential(
                (nn.LayerNorm if not rms_norm else RMSNorm)(embed_dim, eps=norm_epsilon, **factory_kwargs),
    ...
    def forward_features(self, x, inference_params=None):
        ...
        # return only cls token and distillation features
        if self.model_type == "student":
            feats = self.distill_head(hidden_states)
            feats = feats / feats.norm(dim=-1, keepdim=True)
        else:
            feats = hidden_states / hidden_states.norm(dim=-1, keepdim=True)
        return hidden_states[:, 0, :], feats

    def forward(self, x, inference_params=None):
        x, feats = self.forward_features(x, inference_params)
        x = self.head(x)
        return x, feats
    

@register_model
def videomamba_middle_distill(pretrained=False, **kwargs):
    model = VisionMamba(
        patch_size=16, 
        embed_dim=576,
        depth=32, 
...
if __name__ == '__main__':
    ...
    model = videomamba_middle_distill().cuda()
