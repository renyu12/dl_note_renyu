...
JOB_NAME='videomamba_small_res224to448to576'
...
# renyu: 训练small 576*576升分辨率模型，基于small 448*448进行fine tune（相当于一步一步微调增加分辨率），用的8卡服务器srun提交的
srun --mpi=pmi2 \
    -p ${PARTITION} \
    -n${NNODE} \
    --gres=gpu:${NUM_GPUS} \
    ...
    python -m torch.distributed.launch --nproc_per_node=${NUM_GPUS} --use_env main.py \
        ...
        --finetune your_model_path/videomamba_small_res224to448.pth \
        --input-size 576 \
        --batch-size 64 \
        --num_workers 16 \
        --lr 5e-6 \
