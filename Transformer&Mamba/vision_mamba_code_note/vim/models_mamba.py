...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, stride=16, in_chans=3, embed_dim=768, norm_layer=None, flatten=True):
        super().__init__()
        img_size = to_2tuple(img_size)    # renyu: 224->(224,224)
        patch_size = to_2tuple(patch_size)    # renyu: 16->(16,16)
        self.img_size = img_size
        self.patch_size = patch_size
        self.grid_size = ((img_size[0] - patch_size[0]) // stride + 1, (img_size[1] - patch_size[1]) // stride + 1)    # renyu: 图像按指定patch大小和步长会分成几乘几的网格块
        self.num_patches = self.grid_size[0] * self.grid_size[1]
        self.flatten = flatten

        self.proj = nn.Conv2d(in_chans, embed_dim, kernel_size=patch_size, stride=stride)    # renyu: 投影就是做一层卷积
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()    # renyu: 再做归一化
...
def create_block(
    ...
    # renyu: 如果if_bimamba参数为True，就用双向Mamba块
    if if_bimamba:
        bimamba_type = "v1"
    if ssm_cfg is None:
        ssm_cfg = {}
...
# renyu: 会根据运行main函数时输入的--model参数不同创建不同大小的VisionMamba模型
@register_model
